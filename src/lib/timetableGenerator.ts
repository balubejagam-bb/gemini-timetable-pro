...
export class ClientTimetableGenerator {
  ...
  async generateTimetable(
    ...
    error?: string;
    // Newly exposed: fully detailed entries for consumer UIs (optional)
    ...
    try {
      ...
      if (!selectedDepartments || selectedDepartments.length === 0) {
        // Allow advanced mode if subjects OR sections are supplied
        const noSubjects = !options?.subjects || options.subjects.length === 0;
        const noSections = !options?.sections || options.sections.length === 0;
        if (!options?.advancedMode || (noSubjects && noSections)) {
          throw new Error('Select at least one Department OR (Subjects / Sections) for generation');
        }
      }
  ...
  validatedEntries = this.enforceMinimumDailyClasses(validatedEntries, data, selectedSemester);
      ...
      if (validatedEntries.length > 0) {
        ...
        // Build detailed entry objects for front-end snapshot/personalization usage
        const dayLabels = ['Mon','Tue','Wed','Thu','Fri','Sat'];
        const timeSlotMap: Record<number,{start:string;end:string}> = {
          1:{start:'08:00',end:'08:55'},
          2:{start:'08:55',end:'09:50'},
          3:{start:'10:15',end:'11:10'},
          4:{start:'11:10',end:'12:05'},
          5:{start:'12:05',end:'13:00'}
        };
        ...
        const sectionsById = new Map(data.sections.map(sec=>[sec.id,sec]));
        const detailed = validatedEntries.map(e=>{
          const slot = timeSlotMap[e.time_slot];
          const subj = subjectsById.get(e.subject_id);
          const stf = staffById.get(e.staff_id);
          const room = roomsById.get(e.room_id);
          const sec = sectionsById.get(e.section_id);
          return {
            day: dayLabels[e.day_of_week-1] || String(e.day_of_week),
            start_time: slot?.start || '',
            end_time: slot?.end || '',
            subject_code: subj?.code,
            subject_name: subj?.name,
            faculty_name: stf?.name,
